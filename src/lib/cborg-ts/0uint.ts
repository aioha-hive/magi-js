 ...
 * @param {Token} token
 */
export function encodeUint(buf: Bl, token: Token) {
 ...
 */
export function encodeUintValue(buf: Bl, major: number, uint: number | bigint) {
 ...
 * @returns {number}
 */
encodeUint.encodedSize = function encodedSize(token: Token): number {
 ...
 * @returns {number}
 */
encodeUintValue.encodedSize = function encodedSize(uint: number): number {
  if (uint < uintBoundaries[0]) {
    return 1
  }
 ...
 * @returns {number}
 */
encodeUint.compareTokens = function compareTokens(tok1: Token, tok2: Token): number {
