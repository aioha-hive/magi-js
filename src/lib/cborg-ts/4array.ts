 ...
 * @returns {Token}
 */
function toToken(_data: Uint8Array, _pos: number, prefix: number, length: number): Token {
 ...
 * @param {Token} token
 */
export function encodeArray(buf: Bl, token: Token) {
 ...
 * @returns {number}
 */
encodeArray.encodedSize = function encodedSize(token: Token): number {
