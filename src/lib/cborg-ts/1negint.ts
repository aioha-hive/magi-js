 ...
 * @param {Token} token
 */
export function encodeNegint(buf: Bl, token: Token) {
 ...
 * @returns {number}
 */
encodeNegint.encodedSize = function encodedSize(token: Token): number {
  const negint = token.value
  const unsigned = typeof negint === 'bigint' ? negint * neg1b - pos1b : negint * -1 - 1
  /* c8 ignore next 4 */
 ...
 * @returns {number}
 */
encodeNegint.compareTokens = function compareTokens(tok1: Token, tok2: Token): number {
